...
def pos_pred_sample(tree_model, samples, threshold=0.5, use_prob=False):
    """获取预测为正例的样本"""
    ...
    pred_res = []
    for row_idx in tqdm(samples_list, desc=f"filtering positive samples...", total=total_length):
...
def get_samples_shap_val(
        ...
        approximate=False, tree_limit=None
):
    """获取正例样本的 shap 值
    ...
    print(f"Use the `{shap_name}` method.")
    ...
    if target is not None:
        ...
        pos_samples = samples.iloc[pos_index, :].reset_index(drop=True)
    print(f"预测正类数量为: {pos_samples.shape[0]}")
    ...
    if shap_name == 'fast':
        ...
        for ps in tqdm(pos_sample_list, desc=f"{shap_name} method to compute shap value"):
            shap_res.append(
                check_shap_val(
    ...
    print("positive samples shapley values concatenating...")
    shap_values = np.concatenate(shap_res, axis=0)
    print("shap vals shape: ", shap_values.shape)
